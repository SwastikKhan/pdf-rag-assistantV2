...
def create_vector_store(nodes: List[TextNode], store_path: str):
...
def initialize_vector_store_from_upload(uploaded_files):
    with st.spinner("Processing PDFs... Please wait."):
        ...
        # Create a unique temporary folder for the vector store
        store_path = tempfile.mkdtemp(prefix="chroma_db_")
        vector_index = create_vector_store(nodes, store_path=store_path)
...
def get_context(question: str, store_path: str) -> List[str]:
    chroma_client = chromadb.PersistentClient(path=store_path)
...
def chat_interface():
    ...
    if user_input:
        # Retrieve the vector store path from session_state
